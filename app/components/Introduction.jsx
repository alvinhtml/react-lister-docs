...
const installCode = `npm install react-lister --save`
const jsonCode = `[{
  "id": 1,
  "name": "alvin",
...
const columnCode = `import Lister, {Column, withLister} from 'react-lister';
...
const completeCode = `const options = {
  order: true, // 是否需要排序
  visibility: true, // 是否可见
  width: 180, // 列表的初始宽度，默认 200
  resize: true // 是否允许拖动改变列宽度
...
const listerCode = `import * as React from 'react';
...
`
const userCode = `import React, {Component, useState, useEffect} from 'react';
  ...
  render() {
    ...
    return (
      <div className="markdown-section">
        ...
        <h2>安装</h2>
        <pre>
          <code className="language-javascript">{installCode}</code>
        </pre>
        <h2>使用方法</h2>
        ...
        <pre>
          <code className="language-javascript">{columnCode}</code>
        </pre>

        <p>完使代码如下：</p>
        <pre>
          <code className="language-javascript">{completeCode}</code>
        </pre>
        <p>userColumns 代表了我们每一列的配置，接下来需要将 userColumns 传递给 withLister 高阶组件，<code>List.jsx</code> 完整代码：</p>
        <pre>
          <code className="language-javascript">{listerCode}</code>
        </pre>
        ...
        <pre>
          <code className="language-javascript">{paginationCode}</code>
        </pre>
